...
AgriAnalyze is implemented as a six-layer precision agriculture system, deployed as a web application with a FastAPI (Python 3.11) backend and a React + Vite + TailwindCSS frontend. The architectural layers are organised as follows:
...
A systematic ablation study of hybrid deep-learning pipelines for drone-based crop disease detection in Indian agriculture has been presented. The AgriAnalyze system was evaluated across three pipeline configurations on 935 test images spanning 21 wheat and rice disease classes.
