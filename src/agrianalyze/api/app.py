"""FastAPI application factory and HTTP/WebSocket route definitions for AgriAnalyze.

This module hosts the entire backend surface area of AgriAnalyze:
...
Usage:
    uvicorn agrianalyze.api.app:get_app --factory --host 127.0.0.1 --port 9000
"""
import asyncio
import base64
import csv
...
#   CORS middleware            (env-driven, see AGRIANALYZE_CORS_* vars)
...
def create_app() -> FastAPI:
    ...
    #   AGRIANALYZE_CORS_ORIGINS  — comma-separated list of allowed origins
    #                             (e.g. "http://localhost:5173,https://my-app.vercel.app")
    #                             Default: localhost dev origins only.
    #   AGRIANALYZE_CORS_ALLOW_CREDENTIALS — "true"/"false" (default: false)
    _default_dev_origins = (
        "http://localhost:5173,http://127.0.0.1:5173,"
        "http://localhost:3000,http://127.0.0.1:3000"
    )
    _origins_raw = os.environ.get("AGRIANALYZE_CORS_ORIGINS", _default_dev_origins)
    _allow_origins = [o.strip() for o in _origins_raw.split(",") if o.strip()]
    _allow_credentials = os.environ.get(
        "AGRIANALYZE_CORS_ALLOW_CREDENTIALS", "false"
    ).lower() in ("1", "true", "yes")
    ...
    async def get_training_logs():
        ...
        log_paths = [
            ...
            project_root / "outputs" / "logs" / "agrianalyze.log",
        ]
        for lp in log_paths:
            if lp.is_file():
                try:
    ...
    async def get_matrix_progress():
        ...
        # Google Drive for desktop (Windows): "G:/My Drive/agri-analyze/results_v2/matrix"
        drive_candidates = [
            Path(r"G:/My Drive/agri-analyze/results_v2/matrix"),
            Path(r"H:/My Drive/agri-analyze/results_v2/matrix"),
            Path(os.path.expanduser("~/My Drive/agri-analyze/results_v2/matrix")),
            Path(os.path.expanduser("~/Google Drive/agri-analyze/results_v2/matrix")),
        ]
    ...
    async def dataset_stats():
        ...
        # ── Cloud datasets on HuggingFace (ashu010/agrianalyze-data) ──
        # Listed read-only so the UI can show the full 75k collection even when
        # nothing is downloaded locally. Safe to fail silently (offline / no HF).
        # Disk-cached (survives uvicorn --reload) + stale-while-revalidate.
        try:
            import json
            import time
            import threading

            cache_file = _datasets_root() / ".hf_cache.json"
            now = time.time()
            ...
            if cache_file.exists():
                try:
                    ...
                    hf_entries = cached.get("entries", [])
                    cache_age = now - cached.get("ts", 0)
                except Exception:
                    cache_age = float("inf")

            def _refresh_hf_cache() -> None:
                try:
                    from huggingface_hub import HfApi
                    from collections import Counter, defaultdict
                    files = HfApi().list_repo_files(
                        "ashu010/agrianalyze-data", repo_type="dataset"
                    )
                    counts: Counter = Counter()
                    classes_per_folder: dict[str, set[str]] = defaultdict(set)
                    IMG_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}
                    for f in files:
                        if "/" not in f:
                            continue
                        top, rest = f.split("/", 1)
                        if Path(rest).suffix.lower() not in IMG_EXTS:
                            continue
                        counts[top] += 1
                        if "/" in rest:
                            classes_per_folder[top].add(rest.split("/", 1)[0])
                    HF_BASE = (
                        "https://huggingface.co/datasets/"
                        "ashu010/agrianalyze-data/tree/main"
                    )
                    new_entries = [{
                        "name": folder,
                        "path": f"huggingface.co/ashu010/agrianalyze-data/{folder}",
                        "writable": False,
                        "source": "huggingface",
                        "url": f"{HF_BASE}/{folder.replace(' ', '%20')}",
                        "total_images": n,
                        "total_bytes": 0,
                        "classes": [
                            {"name": c, "n_images": 0, "size_mb": 0}
                            for c in sorted(classes_per_folder.get(folder, set()))
                        ],
                    } for folder, n in sorted(counts.items(), key=lambda x: -x[1])]
                    cache_file.parent.mkdir(parents=True, exist_ok=True)
                    cache_file.write_text(
                        json.dumps({"ts": time.time(), "entries": new_entries}),
                        encoding="utf-8",
                    )
                except Exception as exc:
                    logger.debug("HF cache refresh failed: %s", exc)

            # Serve whatever we have instantly; refresh in background if stale (>24h)
            if not hf_entries:
                # First-ever call — must block once
                _refresh_hf_cache()
                if cache_file.exists():
                    hf_entries = json.loads(cache_file.read_text(encoding="utf-8")).get("entries", [])
            elif cache_age > 86400:
                threading.Thread(target=_refresh_hf_cache, daemon=True).start()

            datasets_info.extend(hf_entries)
        except Exception as exc:  # pragma: no cover
            logger.debug("HF dataset listing skipped: %s", exc)
...
# Module-level __getattr__ so `from agrianalyze.api.app import app` and
# uvicorn agrianalyze.api.app:app both work without eager creation.
def __getattr__(name):
    """Expose ``app`` as a module-level attribute built on first access.

    Enables both ``uvicorn agrianalyze.api.app:app`` and
    ``from agrianalyze.api.app import app`` without creating the app at
    import time.
