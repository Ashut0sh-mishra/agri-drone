...
nb1 = [
    ...
    _md("## 3. Data source",
        "",
        "**Default: Kaggle** — zero Drive space, pulls PlantVillage directly into the Colab VM.",
    ...
    _code("#@title Data source",
          ...
          "        print('Upload kaggle.json (Kaggle -> Settings -> Create Legacy API Key)')",
          "        up = files.upload()",
          "        os.makedirs('/root/.kaggle', exist_ok=True)",
          "        with open('/root/.kaggle/kaggle.json','wb') as f: f.write(list(up.values())[0])",
          "        os.chmod('/root/.kaggle/kaggle.json', 0o600)",
          "    !pip install -q kaggle",
          "    !kaggle datasets download -d $KAGGLE_SLUG -p datasets/externals --unzip",
          "    print('Kaggle dataset staged at datasets/externals/')",
          "elif DATA_SOURCE == 'gdrive':",
          "    assert os.path.isdir(DRIVE_DATA), f'Missing {DRIVE_DATA}. Upload dataset first.'",
          "    !ln -sfn \"$DRIVE_DATA\" datasets/externals/drive",
          "elif DATA_SOURCE == 'zenodo':",
          "    ZENODO_URL = '' #@param {type:'string'}",
          "    assert ZENODO_URL, 'Set ZENODO_URL to your archive link'",
          "    !wget -q $ZENODO_URL -O /tmp/zenodo.zip && unzip -q /tmp/zenodo.zip -d datasets/externals/",
    ...
    _code("import shutil, time",
          ...
          "print('  gh pr comment main --body-file RESULTS_SUMMARY.md')"),
]
