/**
 * App.jsx — AgriAnalyze frontend root component.
 *
 * This is a single-file SPA shell. It holds ALL global state and renders one
 * of several "pages" based on `activePage`. Pages are inline render functions
 * (not separate routes) so they can all share the same state hooks without
 * prop drilling.
 *
 * FILE MAP (search for the banner text to jump to each section):
 *   IMPORTS                    — component + service imports
 *   GLOBAL STATE               — useState hooks (upload, multi, video, camera, stats...)
 *   API BOOTSTRAP              — useEffect: discover backend port, health check
 *   FILE HANDLERS              — handleFileSelect, handleMultiFileSelect, handleVideoSelect
 *   DRAG-AND-DROP              — handlePageDrop + page-wide listeners
 *   DETECTION HANDLERS         — handleSubmit, handleMultiDetection, handleVideoDetection
 *   DOWNLOAD HELPERS           — downloadJSON, downloadCSV (export current result)
 *   PAGE RENDERERS             — renderUpload, renderLive, renderFieldMap, renderReports,
 *                                renderDataset, renderPage (the dispatcher)
 *   ROOT JSX                   — <Navbar/> + <Sidebar/> + <main>{renderPage()}</main>
 *
 * Why one file? — Pages share ~25 pieces of state (selectedFile, result, scanHistory,
 * apiHealthy, multiFiles, videoResults, etc.). Splitting into routed pages would
 * require lifting all of it to a context, which is a larger refactor.
 */
...
export default function App() {
  ...
  // ════════════════════════════════════════════════════════════════════════
  // API BOOTSTRAP — find backend port, verify /health on mount
  // ════════════════════════════════════════════════════════════════════════
  useEffect(() => {
    const initAPI = async () => {
      await findAPI()
  ...
  // ════════════════════════════════════════════════════════════════════════
  // FILE HANDLERS — single image, batch images, video
  // ════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════
  // DRAG-AND-DROP — page-wide listeners (images/videos dropped anywhere)
  // ════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════
  // DETECTION HANDLERS — single, batch, video (call backend /detect endpoints)
  // ════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════
  // DOWNLOAD HELPERS — export current result as JSON or CSV
  // ════════════════════════════════════════════════════════════════════════
  /* ── Export helpers ── */
  const downloadJSON = () => {
    if (!result) return
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // PAGE DISPATCHER — maps activePage string to its renderer component
  // ═══════════════════════════════════════════════════════════════════════════
  const renderPage = () => {
    switch (activePage) {
  ...
  /* ════════════════════════════════════════════
     ROOT JSX LAYOUT — Navbar + Sidebar + <main>{renderPage()}</main>
     Drop listeners on the outer div enable page-wide drag-and-drop upload.
     ════════════════════════════════════════════ */
  return (
    <div
