...
const ACCEPTED_FORMATS = 'image/jpeg,image/jpg,image/png,image/gif,image/bmp,image/webp,image/heic,image/heif,image/*,video/mp4,video/avi,video/mov,video/webm,video/mkv,video/*'

export default function UploadBox({ onFileSelect, onMultiFileSelect, onVideoSelect, onCameraClick, selectedCrop, setSelectedCrop, areaAcres, setAreaAcres, growthStage, setGrowthStage, disabled }) {
  ...
  const onDrop = (e) => {
    ...
    const images = allFiles.filter(f => f.type.startsWith('image/') || /\.(heic|heif|bmp|webp)$/i.test(f.name))
    if (videos.length > 0 && onVideoSelect) {
      onVideoSelect(videos[0])
    } else if (images.length > 1 && onMultiFileSelect) {
      onMultiFileSelect(images)
      ...
      >
        ...
        <div className="text-center">
          <p className="font-semibold text-base" style={{ color: 'var(--text-primary)' }}>
            Drop field photos or video here
          </p>
          <p className="text-xs mt-1" style={{ color: 'var(--text-faint)' }}>
            or click to browse &middot; Images: JPG, PNG, WebP, HEIC &middot; Video: MP4, AVI, MOV &middot; Multiple files OK
          </p>
        </div>
      </div>
