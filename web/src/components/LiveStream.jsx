...
export default function LiveStream() {
  ...
  const startStream = useCallback(async () => {
    ...
    ws.onerror = () => {
      const isCloud = apiUrl.includes('.hf.space') || apiUrl.includes('huggingface')
      setError(
        isCloud
          ? 'WebSocket not supported on HF Spaces free tier. Live streaming requires a local backend (localhost).'
