# AgriDrone Complete System - Setup & Integration Guide
...
### Backend (`d:\Projects\agri-drone`)

**Technology**: Python 3.11 + FastAPI + YOLOv8
**Port**: 8000

**Key Files**:
- `src/agridrone/api/app.py` - FastAPI application
- `src/agridrone/api/routes/detection.py` - Detection endpoints
- `src/agridrone/api/schemas.py` - Pydantic models
- `src/agridrone/vision/infer.py` - YOLOv8 detector
- `src/agridrone/vision/postprocess.py` - Detection filtering
...
### Frontend (`d:\Projects\agri-drone-frontend`)
...
# Verify all imports
python -c "from src.agridrone.api.app import create_app; print('OK')"
```
...
1. Edit Python files in `src/agridrone/`
...
```bash
# Build Docker image
docker build -t agridrone:1.0.0 .

# Run container
docker run -p 8000:8000 agridrone:1.0.0

# Or use Gunicorn
pip install gunicorn
gunicorn agridrone.api.app:app -w 4 -b 0.0.0.0:8000
```

#### Enable CORS for Production

In `src/agridrone/api/app.py`:
...
```bash
# View recent logs
tail -f outputs/logs/agridrone.log
...
**A**: Yes, replace the YOLOv8 detector in `src/agridrone/vision/infer.py`:
...
- **Backend**: `d:\Projects\agri-drone\docs\api.md`
- **Frontend**: `d:\Projects\agri-drone-frontend\README.md`
- **QuickStart**: `d:\Projects\agri-drone-frontend\QUICKSTART.md`
- **Architecture**: `d:\Projects\agri-drone\docs\architecture.md`

### Getting Help

1. **Check logs**:
   - Backend: `outputs/logs/agridrone.log`
...
- **Backend**: AgriDrone v1.0.0 (FastAPI + YOLOv8)
- **Frontend**: AgriDrone v1.0.0 (React + Vite + Tailwind)
- **Python**: 3.11+
- **Node**: 16+ with npm
...
**Enjoy using AgriDrone for intelligent crop protection! 🌾🚜🤖**
